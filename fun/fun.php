...
function mp_matricula(){
    ...
    $tsql = "SELECT 
            ...
            cvegrupo, 
            califfinal, 
            convert(varchar, fechaactsis, 1) AS fecha
            from alumnomodulo
            where cveunidadmin = {$_GET["cveunidadmin"]}
                and cvemodeloeducativo = {$_GET["cvemodeloeducativo"]}
                and calendario = 'S'
                and matricula = '{$_GET["mp_matricula"]}'
                and califfinal < 7
                and periodoescolar <> 11819";
    $cadena = "0";
    $getResults= sqlsrv_query($conn, $tsql);
    while ($row = sqlsrv_fetch_array($getResults, SQLSRV_FETCH_ASSOC)) {
        $cadena = $cadena . "&" . $row['periodoescolar'] . "|" . $row['Alumno'] . "|" . $row['desmodulo'] . "|" . $row['cvegrupo'] . "|" . $row['califfinal'] . "|" . $row['fecha'];
    }
    echo ($cadena);
    sqlsrv_free_stmt($getResults);
}
